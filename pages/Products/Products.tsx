...
const Products: React.FC<ProductsPageProps> = ({
  ...
  const [sortBy, setSortBy] = useState<string>(SORT_OPTIONS.FEATURED);
  const [viewMode, setViewMode] = useState<"grid" | "list">("grid");
  const { state } = useCart();

  const brands = [
    "All",
    ...Array.from(new Set(allProducts.map((p) => p.brand).filter(Boolean))),
  ];
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-white via-blue-50/60 to-blue-100/40 pt-28">
      <div className="bg-white/80 backdrop-blur-sm border-b border-gray-200/50 pt-4">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          ...
          <div className="flex items-center space-x-4">
            ...
            >
              ...
              <option value={SORT_OPTIONS.PRICE_HIGH}>
                Price: High to Low
              </option>
